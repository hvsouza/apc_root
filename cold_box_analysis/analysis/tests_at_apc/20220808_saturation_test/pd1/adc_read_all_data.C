...
void adc_read_all_data(){
    ...
    r.baselineTime = 20; // time limit for baseline
    r.chargeTime = 4; // last time to integrate 
    r.startCharge = 0;
    r.maxRange = 4; // max range to search for amplitude peak
    r.fast = 200; // fprompt fast integration time
    r.slow = 1700; //fprompt slow integration time
    r.exclusion_baseline = 0.015; // filtered waveform, anything above here will do +exclusion window
