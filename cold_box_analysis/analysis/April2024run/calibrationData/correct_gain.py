...
def save_plot_data(module, channels, xv, yv, xl, yl):
    sof = [ 1 if ch == 0 else 2 for ch in channels ]
    for s, x, y in zip(sof, xv, yv):
        if module == "VD" or module == "HD":
            _label = f'{module}'
        ...
        with open(f"{module}_SoF_{s}_{yl}_vs_{xl}_4k.csv","w") as f:
...
if __name__ == '__main__':
    ...
    d = np.genfromtxt("/home/henrique/Dropbox/APC_Paris/Root/daphne_gain/vgain.txt")
    vgainsref = d[:,0]
    gainref = d[:,1]
    gainref = np.array([ 10**(g/20) for g in gainref])
    warmgain = 1e3
    spes_corrected = [[] for _ in channels]
    for i, ch in enumerate(channels):
        vgains[i], snr[i], drange[i], spes[i] = retrieve_info(module, ch)

        spes_corrected[i] = [ v*2e6/2**14 for v in spes[i] ]
        print('Gain: ', end='')
        for j, vg in enumerate(vgains[i]):
            idx_ = np.searchsorted(vgainsref, vg)
            g = gainref[idx_]
            spes_corrected[i][j] /= g
            spes_corrected[i][j] /= warmgain
            print(f"{g:.2f}", end=' ')
        print()
        print(f"SPE {ch}: ", end='')
        for s in spes_corrected[i]:
            print(f"{s:.3f}", end=' ')
        print()
        print(f"VGAIN: ", end='')
        for v in vgains[i]:
    ...
    save_plot_data(module, channels, vgains, snr, "vgain", "snr")
    ...
    save_plot_data(module, channels, vgains, spes, "vgain", "spes")
    plt.xlabel(r'$V_{GAIN} [V]$')
    ...
    plt.figure()
    make_plot(module, channels, vgains, spes_corrected)
    plt.xlabel(r'$V_{GAIN} [V]$')
    plt.ylabel(r'Corrected SPE amplitude [$\mu$V]')
    plt.legend(loc='best', frameon=True)
