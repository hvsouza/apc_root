...
void adc_read_all_data(){
    ...
    r.exclusion_baselines = {10, 3, 30, 20, 3, 4, 20, 4}; // filtered waveform, anything above here will do +exclusion window
    r.exclusion_window = 200; // time in ns that it will jump for baseline
    r.filter = 6; // denoise filter.. if filter = 0, no denoise is done. r.OnlyOneEvent = false; // Do you want only one event? Choose it wisely
    // r.OnlyOneEvent = true;
    r.stopEvent = 10000;
    r.noBaseline = false;
    r.polarity = 1;
