...
void giveMeSphe(){
    ...
    dark.nshow_range = {0,200}; // will save some debugging waveforms inside the range.
    ...
    dark.baseLimit    = 6;      // higher then this wont contribute to the baseline abs(baseLimit) (not used for led)

    dark.start  = 0;            // start the search for peaks or start the integration (led)
    dark.finish = 40000;        // fisish the search or finish the integration (led)

    dark.timeLow        = 180;   // integration time before peak (not used for led)
    dark.timeHigh       = 1120;  // integration time after peak (not used for led)
    ...
    dark.too_big        = 600;  // if there is a peak > "too_big" .. wait "waiting" ns for next peak
    dark.waiting        = 2000;
    dark.filter         = 16;   // one dimentional denoise filter (0 equal no filder)
    dark.interactions   = 100;   // for moving avarage filter (not used on led)
    ...
    dark.sphe_charge   = 6423.37; // charge of 1 and 2 p.e. (use fit_sphe.C)
    dark.sphe_charge2  = 12973.8;
    dark.sphe_std      = 930.682;
